...
AMBIENT_KEYWORDS_REGEX = [
    ...
    re.compile(r'\bpeko-chan\b', re.IGNORECASE),
    re.compile(r'\busada\b', re.IGNORECASE) # Added 'usada' as per persona
]
    ...
    async with message.channel.typing():
        try:
            ...
            try: # Response text extraction
                ...
                # NEW: Clean up any Discord-style mentions generated by the AI
                # This removes <@USER_ID> or <@!USER_ID> (user mentions) and <@&ROLE_ID> (role mentions)
                bot_response_text = re.sub(r'<@!?(\d+)>', '', bot_response_text) # Remove user mentions
                bot_response_text = re.sub(r'<@&(\d+)>', '', bot_response_text) # Remove role mentions
